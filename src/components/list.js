...
export default (post) => {
    return (<li className="py-2 flex" key={post.fields.slug}>
                <h1 className="text-xl font-light md:text-base hidden md:block text-gray-400 w-1/12">{getDate(post.frontmatter.date)}</h1>
                <h1 className="text-gray-900 md:text-xl px-6 md:px-0 w-9/12">
                <Link  to={post.fields.slug} className="text-gray-900 listLink font-semibold hover:no-underline">
